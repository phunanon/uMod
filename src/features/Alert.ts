...
export const DeleteAlert: Feature = {
  ...
  async HandleInteractionCreate(interaction) {
    const info = await InteractionGuard(interaction, 'delete-alert', true);
    if (!info) return;
    const { chatInteraction, guildSf, channelSf } = info;
    const { options } = chatInteraction;
    ...
    if (!id) {
      await chatInteraction.reply('Invalid Alert ID.');
      return;
    }
    await chatInteraction.deferReply();
    ...
    if (!alert) {
      await chatInteraction.editReply(
        `Alert ${id} not found for this channel.`,
      );
      return;
    }
    await prisma.alert.delete({ where: { id } });
    await chatInteraction.editReply(`Alert ${id} deleted.`);
  },
};
